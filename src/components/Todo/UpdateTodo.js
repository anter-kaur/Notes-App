...
const UpdateTodo=()=>{
    ...
    useEffect(()=>{
        async function fetchdata(){
            try{
            const response=await axios.get(`http://localhost:2000/api/v1/todo/getsinglepost/${id}`)
    ...
    const submitHandler=async (e)=>{
        e.preventDefault();
        try{
        const response=await axios.patch(`http://localhost:2000/api/v1/todo/updatepost/${id}`,{notes:notes},
